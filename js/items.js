// ============================================================
// items.js — Catálogo completo de itens da Fase 1 (Vila)
// Tipos: material | consumable | weapon | armor | accessory
// Raridade: comum | incomum | raro | épico
// Todos os itens são tradable (preparado para marketplace futuro)
// ============================================================

const RARITY = {
  comum:   { label: 'Comum',   color: '#c8c8c8' },
  incomum: { label: 'Incomum', color: '#4a90d9' },
  raro:    { label: 'Raro',    color: '#f0c419' },
  epico:   { label: 'Épico',   color: '#a855f7' },
};

// ── Catálogo completo ─────────────────────────────────────────
const ITEM_CATALOG = {

  // ── Materiais (drops de mobs, sem efeito agora) ───────────
  forest_herb: {
    id: 'forest_herb', name: 'Erva da Vila',
    type: 'material', rarity: 'comum',
    desc: 'Erva medicinal encontrada nos arredores da vila. Útil para alquimia.',
    value: 5, tradable: true, icon: '🌿', img: 'itm_herb.png',
  },
  goblin_coin: {
    id: 'goblin_coin', name: 'Moeda de Goblin',
    type: 'material', rarity: 'comum',
    desc: 'Moeda surrupiada por goblins. Vale pouco, mas é algo.',
    value: 8, tradable: true, icon: '🪙', img: 'itm_goblin_coin.png',
  },
  bone_fragment: {
    id: 'bone_fragment', name: 'Fragmento de Osso',
    type: 'material', rarity: 'comum',
    desc: 'Osso quebradiço de esqueleto. Pode ter alguma utilidade.',
    value: 6, tradable: true, icon: '🦴', img: 'itm_bone_fragment.png',
  },
  wolf_pelt: {
    id: 'wolf_pelt', name: 'Pele de Lobo',
    type: 'material', rarity: 'incomum',
    desc: 'Pele espessa de lobo dos arredores da vila. Boa para fabricar armaduras.',
    value: 20, tradable: true, icon: '🐺', img: 'itm_wolf_pelt.png',
  },
  wolf_fang: {
    id: 'wolf_fang', name: 'Presa de Lobo',
    type: 'material', rarity: 'incomum',
    desc: 'Presa afiada de lobo. Pode ser usada em amuletos.',
    value: 18, tradable: true, icon: '🦷', img: 'itm_wolf_fang.png',
  },
  orc_tooth: {
    id: 'orc_tooth', name: 'Dente de Orc',
    type: 'material', rarity: 'incomum',
    desc: 'Dente massivo de um orc guerreiro. Assustador.',
    value: 22, tradable: true, icon: '⚔️', img: 'itm_orc_tooth.png',
  },
  armor_shard: {
    id: 'armor_shard', name: 'Fragmento de Armadura',
    type: 'material', rarity: 'raro',
    desc: 'Pedaço de armadura orcana de boa qualidade.',
    value: 45, tradable: true, icon: '🛡️', img: 'itm_shard_metal.png',
  },
  ancient_coin: {
    id: 'ancient_coin', name: 'Moeda Antiga',
    type: 'material', rarity: 'epico',
    desc: 'Moeda de uma civilização esquecida. Colecionadores pagam bem.',
    value: 150, tradable: true, icon: '🏅', img: 'itm_rune_coin.png',
  },

  // ── Consumíveis ───────────────────────────────────────────
  minor_potion: {
    id: 'minor_potion', name: 'Poção Menor de Vida',
    type: 'consumable', rarity: 'comum',
    desc: 'Restaura 40 HP imediatamente.',
    value: 12, tradable: true, icon: '🧪', img: 'itm_small_health_potion.png', imgSize: 60, quickbarSize: 60,
    effect: { type: 'heal', amount: 40 },
  },
  health_potion: {
    id: 'health_potion', name: 'Poção de Vida',
    type: 'consumable', rarity: 'incomum',
    desc: 'Restaura 50 HP imediatamente.',
    value: 30, tradable: true, icon: '⚗️', img: 'itm_small_mana_potion.png',
    effect: { type: 'heal', amount: 50 },
  },
  strength_elixir: {
    id: 'strength_elixir', name: 'Elixir de Força',
    type: 'consumable', rarity: 'raro',
    desc: 'Aumenta ATK em +5 por 60 segundos.',
    value: 55, tradable: true, icon: '🔥', img: 'itm_strength_elixir.png',
    effect: { type: 'buff', stat: 'attack', amount: 5, duration: 60000 },
  },
  xp_elixir: {
    id: 'xp_elixir', name: 'Elixir de XP',
    type: 'consumable', rarity: 'raro',
    desc: 'Concede XP bônus ao herói imediatamente.',
    value: 60, tradable: true, icon: '✨', img: 'itm_xp_elixir.png',
    effect: { type: 'xp', amount: 50 },
  },

  // ── Armas (slot: weapon) ──────────────────────────────────
  wooden_sword: {
    id: 'wooden_sword', name: 'Espada de Madeira',
    type: 'weapon', rarity: 'comum', slot: 'weapon',
    desc: 'Espada talhada em madeira resistente. Melhor que nada.',
    value: 15, tradable: true, icon: '🗡️', img: 'itm_small_wooden_sword.png',
    bonus: { attack: 2 },
  },
  long_wooden_sword: {
    id: 'long_wooden_sword', name: 'Espada Longa de Madeira',
    type: 'weapon', rarity: 'comum', slot: 'weapon',
    desc: 'Espada longa de madeira. Maior alcance.',
    value: 20, tradable: true, icon: '🗡️', img: 'itm_long_wooden_sword.png',
    bonus: { attack: 3 },
  },
  wooden_bow: {
    id: 'wooden_bow', name: 'Arco de Madeira',
    type: 'weapon', rarity: 'comum', slot: 'weapon',
    desc: 'Arco simples de madeira. Ideal para caçadores.',
    value: 18, tradable: true, icon: '🏹', img: 'itm_small_wooden_bow.png', imgSize: 120,
    bonus: { attack: 3 },
  },
  wooden_staff: {
    id: 'wooden_staff', name: 'Cajado de Madeira',
    type: 'weapon', rarity: 'comum', slot: 'weapon',
    desc: 'Cajado para canalizar magia. Útil para magos e clérigos.',
    value: 18, tradable: true, icon: '🪄', img: 'itm_wooden_staff.png',
    bonus: { attack: 3 },
  },
  wooden_mace: {
    id: 'wooden_mace', name: 'Maça de Madeira',
    type: 'weapon', rarity: 'comum', slot: 'weapon',
    desc: 'Maça pesada de madeira. Boa para guerreiros e clérigos.',
    value: 16, tradable: true, icon: '🔨', img: 'itm_wooden_mace.png',
    bonus: { attack: 2 },
  },

  // ── Armaduras (slot: armor) ───────────────────────────────
  leather_armor: {
    id: 'leather_armor', name: 'Armadura de Couro',
    type: 'armor', rarity: 'comum', slot: 'armor',
    desc: 'Proteção básica de couro curtido. Reduz 1–3 de dano por ataque recebido.',
    value: 18, tradable: true, icon: '🧥', img: 'itm_brown_leather_armor.png',
    bonus: { armorReduction: 3 },
  },
  wolf_cape: {
    id: 'wolf_cape', name: 'Capa de Lobo',
    type: 'armor', rarity: 'incomum', slot: 'armor',
    desc: 'Capa feita de pele de lobo. Intimidante e resistente.',
    value: 45, tradable: true, icon: '🦺', img: 'itm_dark_wolf_cape.png',
    bonus: { maxHp: 25, attack: 1 },
  },
  chainmail: {
    id: 'chainmail', name: 'Cota de Malha',
    type: 'armor', rarity: 'raro', slot: 'armor',
    desc: 'Armadura de anéis de ferro entrelaçados. Boa proteção.',
    value: 85, tradable: true, icon: '🛡️', img: 'iron_chainmail_armor.png',
    bonus: { maxHp: 40 },
  },

  // ── Key Gems (consumível especial — uma por classe) ──────
  keygem_warrior: {
    id: 'keygem_warrior', name: 'Pedra do Guerreiro',
    type: 'keygem', rarity: 'epico',
    desc: 'A essência do guerreiro Karveth. Necessária para abrir o portal da Fase 2.',
    value: 0, tradable: false, icon: '🔴', img: 'itm_keygem_warrior.png',
    heroClass: 'warrior',
  },
  keygem_hunter: {
    id: 'keygem_hunter', name: 'Pedra da Caçadora',
    type: 'keygem', rarity: 'epico',
    desc: 'A essência selvagem de Sylvara. Necessária para abrir o portal da Fase 2.',
    value: 0, tradable: false, icon: '🟢', img: 'itm_keygem_hunter.png',
    heroClass: 'hunter',
  },
  keygem_mage: {
    id: 'keygem_mage', name: 'Pedra do Mago',
    type: 'keygem', rarity: 'epico',
    desc: 'A essência arcana de Valdris. Necessária para abrir o portal da Fase 2.',
    value: 0, tradable: false, icon: '🔵', img: 'itm_keygem_mage.png',
    heroClass: 'mage',
  },
  keygem_cleric: {
    id: 'keygem_cleric', name: 'Pedra da Clérига',
    type: 'keygem', rarity: 'epico',
    desc: 'A essência sagrada de Aelys. Necessária para abrir o portal da Fase 2.',
    value: 0, tradable: false, icon: '🟡', img: 'itm_keygem_cleric.png',
    heroClass: 'cleric',
  },

  // ── Acessórios (slot: accessory) ──────────────────────────
  forest_amulet: {
    id: 'forest_amulet', name: 'Amuleto da Vila',
    type: 'accessory', rarity: 'incomum', slot: 'accessory',
    desc: 'Amuleto imbuído com energia protetora da vila.',
    value: 40, tradable: true, icon: '📿', img: 'itm_green_forest_amulet.png',
    bonus: { attack: 2, maxHp: 10 },
  },
  fang_necklace: {
    id: 'fang_necklace', name: 'Colar de Presas',
    type: 'accessory', rarity: 'raro', slot: 'accessory',
    desc: 'Colar feito de presas de lobo. Aumenta a ferocidade.',
    value: 75, tradable: true, icon: '📿', img: 'itm_fangs_necklace.png',
    bonus: { maxHp: 20 },
  },
  forest_key: {
    id: 'forest_key', name: 'Chave do Portão da Floresta',
    type: 'consumable', rarity: 'epico',
    desc: 'Uma chave antiga que abre o caminho para a Floresta. Use para avançar para a Fase 2.',
    value: 0, tradable: false, icon: '🗝️', img: 'itm_forest_key.png',
    effect: { type: 'phase2' },
  },

  soul_ring: {
    id: 'soul_ring', name: 'Anel da Alma',
    type: 'accessory', rarity: 'epico', slot: 'accessory',
    desc: 'Anel imbuído com uma gema espectral. Poder misterioso.',
    value: 180, tradable: true, icon: '💍', img: 'itm_gold_ring_purple.png',
    bonus: { attack: 5, maxHp: 30 },
  },
};

// ── Tabela de drops por tipo de mob ──────────────────────────
const MOB_DROP_TABLE = {
  goblin: [
    { itemId: 'forest_herb',   chance: 0.10 },
    { itemId: 'goblin_coin',   chance: 0.05 },
    //{ itemId: 'forest_key',    chance: 0.00 },
    //{ itemId: 'wooden_sword',  chance: 0.12 },
    //{ itemId: 'minor_potion',  chance: 0.08 },
    //{ itemId: 'ancient_coin',  chance: 0.02 },
  ],
  wolf: [
    { itemId: 'forest_herb',   chance: 0.10 },
    { itemId: 'wolf_pelt',     chance: 0.05 },
    { itemId: 'wolf_fang',     chance: 0.05 },
    //{ itemId: 'wolf_cape',     chance: 0.10 },
    { itemId: 'minor_potion',  chance: 0.02 },
    //{ itemId: 'fang_necklace', chance: 0.05 },
  ],
  orc: [
    { itemId: 'orc_tooth',     chance: 0.40 },
    { itemId: 'armor_shard',   chance: 0.25 },
    { itemId: 'chainmail',     chance: 0.06 },
    { itemId: 'health_potion', chance: 0.12 },
    { itemId: 'ancient_coin',  chance: 0.03 },
  ],
  skeleton: [
    { itemId: 'bone_fragment', chance: 0.50 },
    { itemId: 'armor_shard',   chance: 0.15 },
    { itemId: 'soul_ring',     chance: 0.04 },
    { itemId: 'ancient_coin',  chance: 0.03 },
  ],
  goblin_rider: [
    { itemId: 'forest_key',    chance: 0.30 },
    { itemId: 'wooden_bow',    chance: 0.70 },
    { itemId: 'minor_potion',  chance: 0.40 },
  ],
};

// ── Receitas de craft ─────────────────────────────────────────
// id: identificador único da receita (usado para descobertas)
// ingredients: [{ id, qty }] — até 3 ingredientes
// result: id do item resultante
// successChance: 0-1
const CRAFT_RECIPES = [
  {
    id: 'recipe_minor_potion',
    ingredients: [{ id: 'forest_herb', qty: 10 }],
    result: 'minor_potion',
    successChance: 1.00,
  },
  {
    id: 'recipe_leather_armor',
    ingredients: [{ id: 'wolf_pelt', qty: 10 }],
    result: 'leather_armor',
    successChance: 1.00,
  },
  {
    id: 'recipe_fang_necklace',
    ingredients: [
      { id: 'forest_herb', qty: 5  },
      { id: 'wolf_fang',   qty: 10 },
    ],
    result: 'fang_necklace',
    successChance: 1.00,
  },
];

// ── API pública ───────────────────────────────────────────────
const Items = {

  get(id) {
    return ITEM_CATALOG[id] ?? null;
  },

  getRarityColor(rarity) {
    return RARITY[rarity]?.color ?? '#fff';
  },

  // Rola drops para um mob específico pelo tipo
  rollDrops(mobType = 'goblin') {
    const table = MOB_DROP_TABLE[mobType] ?? MOB_DROP_TABLE.goblin;
    const drops = [];
    for (const entry of table) {
      if (Math.random() < entry.chance) {
        const item = ITEM_CATALOG[entry.itemId];
        if (item) drops.push({ ...item }); // cópia para não mutar o catálogo
      }
    }
    return drops;
  },

  // Aplica bônus de um equipável no hero
  applyBonus(hero, item) {
    if (!item.bonus) return;
    if (item.bonus.attack)        hero.attack        += item.bonus.attack;
    if (item.bonus.maxHp)         { hero.maxHp += item.bonus.maxHp; hero.hp += item.bonus.maxHp; }
    if (item.bonus.armorReduction) hero.armorReduction = (hero.armorReduction ?? 0) + item.bonus.armorReduction;
  },

  removeBonus(hero, item) {
    if (!item.bonus) return;
    if (item.bonus.attack)        hero.attack        = Math.max(1, hero.attack - item.bonus.attack);
    if (item.bonus.maxHp)         { hero.maxHp = Math.max(1, hero.maxHp - item.bonus.maxHp); hero.hp = Math.min(hero.hp, hero.maxHp); }
    if (item.bonus.armorReduction) hero.armorReduction = Math.max(0, (hero.armorReduction ?? 0) - item.bonus.armorReduction);
  },
};
